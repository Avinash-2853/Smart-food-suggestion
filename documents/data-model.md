...
This document defines the detailed database schema, entity relationships, and data structures for the NextGen Catering Platform.

---

## Database: Aurora PostgreSQL

### Schema: `catering_platform`
...
- **Foreign Keys:** All relationships enforced with foreign keys
- **Cascade Deletes:** Where appropriate (order_items, order_status_history)
- **Soft Deletes:** Users, restaurants, menu_items use `deleted_at`
- **Unique Constraints:** Email, order_number, ticket_number
- **Check Constraints:** Ratings (1-5), positive amounts
...
- `(restaurant_id, menu_type)` on menu_items

### GIN Indexes (JSONB/Arrays)
- `preferences` on user_profiles
- `dietary_tags`, `tags`, `search_keywords` on menu_items
- `address`, `operating_hours`, `delivery_zones` on restaurants

### GiST Indexes (Geospatial)
- Location indexes on `user_addresses` and `restaurants`
...
### Active Data
- **Orders:** 90 days (fast access)
- **Reviews:** Indefinite (business value)
- **Users:** While account active

### Archived Data
- **Orders:** Move to S3 after 90 days
- **Analytics:** Move to data lake (S3)
...
### Caching Strategy
- Cache hot menu data (Redis)
- Cache restaurant availability (Redis)
